...
void rs485_ISR_writer(void) {
	...
	1) «аполн€ем буфер и включаем прерывание по опустошению UDR - UDRE
	2) Ќачинает работать прерывание USART_UDRE_vect
	3) ¬ обработчике прерывани€ вычитаем один байт из size и пытаемс€ записать данные в UDR
	4) ¬ обработчике прерывани€ провер€ем общее количество оставшихс€ байтов. ≈сли не 0, то переходим к пункту 3
	5) выходим из обработчика прерывани€ и ждем следующего
	...
	*/
	...
	if (_rs485_tx_buff.size == 0) {
		...
		USART_UDRE_INTRR_DISABLE(); // данное прерывание при включении будет вызыватьс€ посто€нно, если стоит флаг UDRE!
	}
}
